...
If you know your way around Docker or possibly want to use a different deployment method, you can jump right to the link:/midpoint/install/[installation guide].
...
In case you decide to change the initial password, keep in mind that there is a link:/midpoint/reference/security/authentication/administrator-initial-password/#default-password-policy[password policy] in place.
...
When you learn to work with midPoint, you may want to, for instance, link:/midpoint/reference/admin-gui/resource-wizard/create-resource-using-wizard/[create a resource and import accounts from a CSV file via the CSV connector].
